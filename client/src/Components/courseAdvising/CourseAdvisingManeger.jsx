    ...
    useEffect(() => {
        const fetchInitialRecords = async () => {
            ...
            try {
                ...
                const recordResponse = await fetch(import.meta.env.VITE_API_KEY + "/get_student_record", {
                    method: "POST",
                    body: formBody,
                ...
                const courseResponse = await fetch(import.meta.env.VITE_API_KEY + "/get_advising_history", {
                    method: "POST",
                    body: formBody,
                    headers: { "content-type": "application/json" },
                });
                if (courseResponse.ok) {
                    ...
                    const prerequisites = result.data.filter(course => course.level < 400);
                    const coursePlan = result.data.filter(course => course.level >= 390);
    ...
    return (
        <form onSubmit={handleSubmit}>
            ...
            <AddPrereqs
                ...
                handleChange={(index, value, field) => handleSectionChange(index, value, field, prerequisites, setPrerequisites)}
            />
            ...
            <AddCourse
                ...
                handleChange={(index, value, field) => handleSectionChange(index, value, field, coursePlan, setCoursePlan)}
            />
